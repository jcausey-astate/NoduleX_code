...
def area_for_polygon(polygon):
    ...
    result += (polygon[imax]["x"] * polygon[0]["y"]) - (
        polygon[0]["x"] * polygon[imax]["y"]
    )
    return abs(result / 2)
...
def centroid_for_polygon(polygon, non_negative=True):
    ...
    if area != 0:
        for i in range(0, imax):
            result_x += (polygon[i]["x"] + polygon[i + 1]["x"]) * (
                (polygon[i]["x"] * polygon[i + 1]["y"])
                - (polygon[i + 1]["x"] * polygon[i]["y"])
            )
            result_y += (polygon[i]["y"] + polygon[i + 1]["y"]) * (
                (polygon[i]["x"] * polygon[i + 1]["y"])
                - (polygon[i + 1]["x"] * polygon[i]["y"])
            )
        result_x += (polygon[imax]["x"] + polygon[0]["x"]) * (
            (polygon[imax]["x"] * polygon[0]["y"])
            - (polygon[0]["x"] * polygon[imax]["y"])
        )
        result_y += (polygon[imax]["y"] + polygon[0]["y"]) * (
            (polygon[imax]["x"] * polygon[0]["y"])
            - (polygon[0]["x"] * polygon[imax]["y"])
        )
...
def get_tumor_centroids(
    ...
    for xfile in xmlfiles:
        ...
        try:
            ...
            eprint(
                "WARNING: XML seems to be missing namespace -- root tag is {0}".format(
                    root.tag
                )
            )
        if dirUID == "":
        ...
        for session in root.iter("{0}readingSession".format(ns)):
            for nodule in session.iter("{0}unblindedReadNodule".format(ns)):
                ...
                if (malignancy_min == None) or (
                    (malignancy != None) and (int(malignancy_min) <= int(malignancy))
                ):
                    for roi in nodule.iter("{0}roi".format(ns)):
                        ...
                        sopUID = (
                            sopUID.text
                            if sopUID != None
                            else missing_sop_uid(
                                "z={0}, nodule={1}".format(zCoord, nID)
                            )
                        )
                        ...
                        for edge in roi.iter("{0}edgeMap".format(ns)):
                            (xCoord, yCoord) = (
                                edge.find("{0}xCoord".format(ns)).text,
                                edge.find("{0}yCoord".format(ns)).text,
                            )
                        ...
                        if area_for_polygon(coords) == 0 and len(coords) > 3:
                            eprint(
                                "WARNING: Annotation with zero area found; nodule ID {0}, z={1}, # of annotation points: {2}".format(
                                    noduleID, zCoord, len(coords)
                                )
                            )
            ...
            if malignancy_min == None:
                for nonNodule in session.iter("{0}nonNodule".format(ns)):
                    ...
                    sopUID = (
                        sopUID.text
                        if sopUID != None
                        else missing_sop_uid("z={0}, nodule={1}".format(zCoord, nID))
                    )
                    if not sopUID in nodulesByFile:
                        nodulesByFile[sopUID] = []
                    ...
                    if locus == None:
                        continue
                    (xCoord, yCoord) = (
                        locus.find("{0}xCoord".format(ns)).text,
                        locus.find("{0}yCoord".format(ns)).text,
                    )
    ...
    for dfile in dicomfiles:
        ...
        file_list.append(
            {
                "sliceNo": sliceNo,
                "fileName": os.path.join(dicomdir, dfile),
                "seriesUID": seriesUID,
            }
        )
        if sopUID in nodulesByFile:
            for center in nodulesByFile[sopUID]:
                file_name_info = (
                    os.path.join(dicomdir, dfile) if not file_name_only else dfile
                )
...
def get_tumor_annotations(
    dicomdir, xmldir=None, file_name_only=False, return_file_list=False
):
...
def print_results(res, dicomdir=None, numeric_only=False):
    if len(res) == 0:
        ...
        eprint(
            "{0}No tumor annotations found corresponding to input dataset.".format(
                location
            )
        )
    else:
...
def run_standalone():
    ...
    while i < len(sys.argv):
        ...
        if token == "-t":
            ...
            matrix_style = True
        i += 1
    res = get_tumor_centroids(
        dicomdir, xmldir, malignancy_min=mal_thresh, file_name_only=file_name_only
    )
    if file_header:
        if matrix_style:
            print(
                "\t".join(
                    ["sliceNo", "x", "y", "z", "isNodule", "inclusion", "malignancy"]
                )
            )
        else:
            print(

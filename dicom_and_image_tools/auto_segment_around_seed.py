...
def get_consensus_for_seed(
    img, nodule_id, output_dir, seed, proximity=10, min_size=None, max_size=None
    ...
    for (sname, strat) in [
        (strnam, segstrats[strnam]["seed-independent"]) for strnam in segstrats
    ]:
        ...
        try:
            ...
            fname = os.path.join(
                output_dir, strat["strategy"].__name__, nodule_id + "-" + optha + ".nii"
            )
    ...
    if seed is not None:
        ...
        if "crop" in img_info:
            ...
            seed = [seed[i] - origin[i] for i in range(len(seed))]
        seeds = [seed]

    consensus_img, seg_info = get_seed_dep_union(
        seed_indep_imgs,
        seeds,
        output_dir,
        nodule_id,
        segstrats,
        img_info["lungseg"]["size"],
        img,
        min_size,
        max_size,
    )
    ...
    for seed in seg_info:
        for segstrat in seg_info[seed]:
            ...
            img_info["noduleseg"].setdefault(seed, {})[segstrat] = combined_info

    return consensus_img, img_info


def get_seed_dep_union(
    imgs,
    seeds,
    root_dir,
    nodule_id,
    ...
    segmented = np.zeros(
        sitk.GetArrayFromImage(imgs.values()[0]).shape  # pylint: disable=E1101
    )
    ...
    for seed in seeds:
        ...
        for (sname, strat) in [
            (strnam, segstrats[strnam]["seed-dependent"]) for strnam in segstrats
        ]:
            img_in = imgs[sname]
        ...
        seed_indep_hashes = [sitkstrats.hash_img(i)[0:8] for i in out_imgs.values()]
        ...
        try:
            ...
            (consensus, consensus_info) = sitkstrats.segmentation_union(
                out_imgs.values(),
                {
                    "threshold": 2.0 / 3.0,
    ...
    strats = {
        "confidence_connected": {
            "seed-independent": {
                ...
                "opts": {"curvature_flow": {"timestep": 0.01, "iterations": 25}},
            },
            "seed-dependent": {
        ...
        "watershed": {
            ...
            "seed-dependent": {"strategy": sitkstrats.isolate_watershed, "opts": {}},
        },
    }
    return strats
...
def configure_seed_driven_strats_conservative():
    ...
    strats = {
        "confidence_connected": {
            "seed-independent": {
                "strategy": sitkstrats.curvature_flow,
                "opts": {"curvature_flow": {"timestep": 0.01, "iterations": 20}},
            },
            "seed-dependent": {
                "strategy": sitkstrats.confidence_connected,
                "opts": {

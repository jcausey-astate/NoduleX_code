...
Usage: 
    {0} [--short | --combine] [--output output_file_name] [--diameter DIAMETER-mm] [--nodules-only] DICOMDIR [DICOMDIR ...]
...
import sys, pydicom as dicom, re, os
import numpy as np
...
def get_args():
    ...
    ap.add_argument(
        ...
        metavar="dicom-dir",
        nargs="+",
        help="Directory (or) containing DICOM source images",
    )
    ap.add_argument(
        "--short",
        dest="short",
        action="store_true",
        help="Use short output form (no patient name)",
    )
    ap.add_argument(
        "--combine",
        dest="combine",
    ...
    ap.add_argument(
        ...
        dest="nodules_only",
        action="store_true",
        help="Only output centroids for nodules with segmentation",
    )
    ap.add_argument(
        "--diameter",
        dest="diameter_mm",
        metavar="diameter-mm",
        required=False,
        help="Diameter (mm) for spheres representing non-nodules; default = 5",
        default=5,
    )
    ap.add_argument(
        "--output",
    ...
    ap.add_argument(
        "--extended-info-file",
        dest="extended_info_file",
        metavar="extended-info-filename",
        required=False,
        help="Filename for extended information on each ROI.",
    )
...
def draw_circle(pixels, center, radius=3, outline=1):
    ...
    if r > 0:
        ...
        ImageDraw.Draw(img).ellipse(
            (x - r, y - r, x + r, y + r), outline=outline, fill=1
        )
        pixels[np.array(img) == 1] = 1
    return pixels
        ...
        for candidate in centroids_by_scan[scan]["centroids"]:
            if "coords" in candidate and len(candidate["coords"]) > 2:
                if scan not in nodules_by_scan:
                    nodules_by_scan[scan] = {
                        "centroids": [],
                        "file_list": centroids_by_scan[scan]["file_list"],
                    }
...
def merge_centroids(centroids_by_scan, non_nodule_diameter=5):
    ...
    for scan in centroids_by_scan:
        ...
        candidates.sort(
            key=lambda x: "coords" in x and len(x["coords"]) > 2, reverse=True
        )
        ...
        for candidate in candidates:
            ...
            if pixels is None:  # If we are initializing this scan...
                ...
                pixels = np.zeros(
                    (n_slices + padding_at_start, img_height, img_width), dtype=int
                )  # holds real segmentations
                cpixels = np.zeros(
                    (n_slices + padding_at_start, img_height, img_width), dtype=int
                )  # holds pseuo-segmentations from non-nodules
                ...
                if (
                    xy_scale == 0
                ):  # Information is sometimes missing; assume unit scale if this happens
                    xy_scale = 1
                if z_scale == 0:
                    z_scale = 1
            ...
            if "coords" in candidate and len(candidate["coords"]) > 2:
                ...
                pix_slice = pix_slice + draw_polygon_from_coords(
                    pix_slice, candidate["coords"]
                )
        ...
        if cpixels is not None:
            n_centroid_groups = measurements.label(
                cpixels, output=cpixels, structure=structure
            )
        if pixels is not None:
            n_segmentation_groups = measurements.label(
                pixels, output=pixels, structure=structure
            )
        ...
        for centroid in centroids:
            ...
            try:
                ...
                if r >= 512 or c >= 512 or r < 0 or c < 0:
                    print(
                        "ILLEGAL ANNOTATION on {} r:{} c:{} s:{} \n{}".format(
                            centroid["patient"], r, c, s, centroid
                        )
                    )
                    ...
                    print(
                        "FAIL on {} r:{} c:{} s:{} \n{}\n{}".format(
                            centroid["patient"], r, c, s, e, centroid
                        )
                    )
                    sys.exit(1)
            if cgroup <= 0:
                ...
                for g_idx, box in enumerate(objects):
                    s_start, s_end = (
                        box[0].start,
                        box[0].stop,
                    )  # objects are given by slices, which have start, stop attributes
                    ulr, lrr = (
                        box[1].start,
                        box[1].stop,
                    )  # one slice per dimension, so we have three: one for slices,
                    ulc, lrc = (
                        box[2].start,
                        box[2].stop,
                ...
                if not was_placed:
                    print(
                        "WARN: centroid did not map to a group! p:{} x:{} y:{} s:{}".format(
                            scan, c, r, s
                        )
                    )
                    print(centroid)
                    continue
            if (
                not cgroup in ignore_cgroups
            ):  # Don't add to groups that overlap segmentations
        ...
        for segmentation in segmentations:
            ...
            if group <= 0:
                ...
                if not was_placed:
                    print(
                        "WARN: polygon did not map to a group! p:{} x:{} y:{} s:{}".format(
                            scan, c, r, s
                        )
                    )
                    continue
...
def output_file_header():
    hdr = "#patient\tsliceNo\tX\tY\tclass\tis_nodule\tinclusion\tmalignancy\n"
    return hdr


def write_candidates(
    centroid_groups_by_scan, output_name=None, combined=False, short=False
):
    output_text = {}
    ...
    for scan in scans:
        dicom_dir = scan
        output_file_name = (
            os.path.join(dicom_dir, output_name) if not combined else output_name
        )
        ...
        for group in centroid_groups:
            # patient Z X Y class is_nodule inclusion malignancy
            output = [
                group["patient"],
                int(round(group["sliceNo"])),
                round(group["x"], 1),
                round(group["y"], 1),
                group["malignancy"],
                group["isNodule"],
    ...
    for output_file in output_text:
        ...
        with open(output_file, "wb") as fout:
            fout.write(output_file_header())
            fout.write("\n".join(output_text[output_file]) + "\n")


def write_extended_information(
    centroid_groups_by_scan, output_name=None, combined=False
):
    output_text = {}
    scans = centroid_groups_by_scan.keys()
    scans.sort()
    for scan in scans:
        ...
        for group in centroid_groups:
            ...
            # Group header:
            # "Consensus" groupID Z X Y class is_nodule inclusion malignancy
            output.append(
                [
                    "Consensus",
                    groupID,
            ...
            # For extended output --- see if the group has a members list; if so, output info for each member.
            if "members" in group:
                members = group["members"]
                members.sort(key=lambda x: x["sliceNo"])
                for member in members:
                    # Member info (group ID, otherwise member-level info)
                    # 'Annot.' groupID Z X Y class is_nodule inclusion malignancy
                    output.append(
                        [
                            "Annot.",
                            groupID,
                            int(round(member["sliceNo"])),
                            round(member["x"], 1),
                            round(member["y"], 1),
                            member["malignancy"],
                            member["isNodule"],
                    ...
                    sliceNo = member["sliceNo"]
                    malignancy = int(member["malignancy"])
                    if malignancy > 0:
                        if sliceNo not in ratings_by_slice:
                            ratings_by_slice[sliceNo] = [0, 0, 0, 0, 0]
                        ratings_by_slice[sliceNo][malignancy - 1] += 1
                        total_ratings += 1
                        rating_sum += malignancy
            # Group statistics
            rated_slice_count = len(ratings_by_slice.keys())
            ratings_per_slice = [sum(ratings_by_slice[k]) for k in ratings_by_slice]
            max_ratings_per_slice = max(ratings_per_slice)
            mode_ratings_per_slice = Counter(ratings_per_slice).most_common(1)[0][0]
            rated_by = {k: sum(ratings_by_slice[k]) for k in ratings_by_slice}
            n_slices_rated_by = [
                sum(1 for x in rated_by.values() if x == n) for n in range(1, 5)
            ]
            n_slices_rated_by.append(sum(1 for x in rated_by.values() if x > 4))
            rating_counts = [
                sum(ratings_by_slice[s][m] for s in ratings_by_slice) for m in range(5)
            ]

            # Summary line example:
            # Summary   groupID    AvgRatingsPerSlice:3.56  ModeRatingsPerSlice:4  MaxRatingsPerSlice:4    RatedSlices:9   TotalRatings:32 Ratings-1,2,3,4,5:0,0,0,9,23    NSlicesRatedBy-1,2,3,4,5+:1,0,1,7,0 AvgRating:4.72
            output.append(
                [
                    "Summary",
                    groupID,
                    "AvgRatingsPerSlice:{}".format(
                        round(total_ratings / float(rated_slice_count), 2)
                    ),
                    "ModeRatingsPerSlice:{}".format(mode_ratings_per_slice),
                    "MaxRatingsPerSlice:{}".format(max_ratings_per_slice),
                    "RatedSlices:{}".format(rated_slice_count),
                    "TotalRatings:{}".format(total_ratings),
                    "Ratings-1,2,3,4,5:{}".format(
                        ",".join([str(x) for x in rating_counts])
                    ),
                    "NSlicesRatedBy-1,2,3,4,5+:{}".format(
                        ",".join([str(x) for x in n_slices_rated_by])
                    ),
                    "AvgRating:{}".format(round(rating_sum / float(total_ratings), 2)),
                ]
            )
            for idx, line in enumerate(output):
                output[idx] = "\t".join([str(x) for x in line])
            if output_file_name in output_text:
                output_text[output_file_name].append(output)
            else:
                output_text[output_file_name] = [output]
    for output_file in output_text:
        print("writing output_file_name: {}".format(output_file))
        with open(output_file, "wb") as fout:
            fout.write("#annotationType" + "\t" + output_file_header()[1:])
            fout.write(
                "\n".join("\n".join(line) + "\n" for line in output_text[output_file])
                + "\n"
            )
...
def run_standalone():
    ...
    write_candidates(
        centroid_groups_by_scan,
        output_name=args["output_file"],
        combined=args["combine"],
        short=args["short"],
    )
    if args["extended_info_file"] is not None:
        write_extended_information(
            centroid_groups_by_scan,
            output_name=args["extended_info_file"],
            combined=args["combine"],
        )

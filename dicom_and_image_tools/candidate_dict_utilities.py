...
def get_group_nodule_flags(group):
    ...
    malignancy = int(
        round(malignancy / float(len(members)))
    )  # Malignancy is the average of all opinions (rounded to nearest int)
...
def set_group_stats(group):
    ...
    group["volume"] = volume
    group["noduleID"] = make_unique_id_string(
        cx, cy, cz, is_nodule, inclusion, malignancy
    )
    return group
...
def nodule_unique_id(candidate, prefix=None):
    ...
    if type(candidate) == type([]):
        ...
        n_id = make_unique_id_string(
            cx, cy, cz, is_nodule, inclusion, malignancy, prefix
        )
    else:
        x = candidate["x"]
        y = candidate["y"]
...
def fill_file_paths(candidates, dicom_dir):
    import pydicom as dicom, dicom_dir_utilities

"""
Adapted from 'masterseg.py' to focus specifically on segmenting the lung
...
def segment_lung_volume(img, probe_size=7, erode=0):  # pylint: disable=C0111
    ...
    """
    lung_img = lungseg.segment_lung(
        sitk.GetImageFromArray(img), {"probe_size": probe_size}
    )
...
def isolate_lungs(img, probe_size=7, remove_lungs=False, erode=0, bg_value=0):
    ...
    return apply_segmentation_to_image(
        img, seg_mask, reverse_mask=remove_lungs, bg_value=bg_value
    )
...
def find_lung_bounds(img, probe_size=7, ignore_trachea=True):
    ...
    if ignore_trachea:
        ...
        for i in range(bounds[0][0], bounds[0][1]):
            ...
            if (
                first is None and count > 1
            ):  # If we have at least 2 objects, one must be a lung.
                first = i
                break

...
def process_command_line(argv):
    ...
    parser.add_argument(
        "--dicomdirs", nargs="+", help="The dicom directories to operate on."
    )
    parser.add_argument("--out_dir", help="The directory to deposit the output files.")
    parser.add_argument(
        "--json", default=None, help="Depost a JSON file with metadata at this path."
    )
...
def convert_to_nii(dicom_in, nifti_dir):
    ...
    Returns sha1 hash of the output file"""
    _, sha = dicom_dir_to_nifti_file(dicom_in, nifti_dir)

    return sha


def dicom_dir_to_nifti_file(dicom_in, nifti_dir):
    """Convert the given dicom directory (dicom_in) into a nifti formatted
    image and place it in nifti_dir using an md5 hash of its contents.
